...
const float driveDerivative = 0.005f;
...
std::vector<realTimePositionController*> realTimePositionControllers = {&left_mtr_back_PID,&right_mtr_back_PID,&right_mtr_front_PID,&left_mtr_front_PID};
...
realTimePositionController* getRealTimePositionController(std::string name) {
  for (realTimePositionController *RTPS : realTimePositionControllers) {
    ...
    if (RTPSS.getName() == name) return RTPS;
  }
...
void setTargetCannonAngle(double newAngle) { //set the angle the cannon wants to be at (degrees)
  (*getRealTimePositionController("cannonAngler")).setTargetPosition(newAngle*5);
  setAPIDIsActivated("cannonAngler",true);
}
